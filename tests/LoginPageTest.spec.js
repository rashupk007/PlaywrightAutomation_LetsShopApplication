...
test.describe("Login Test",()=>{
    test("Check the login with valid credentials", {tag:'@pom1'}, async ({})=>{
        await loginPage.validLogin(username, password)
        await expect(loginPage.homePageIdentifier).toBeVisible()
    })

    test("Check the login with invalid credentials", {tag:'@pom1'}, async ({})=>{
        await loginPage.invalidLogin(username, "password")
        await expect(loginPage.errorMessage).toHaveText("Incorrect email or password.")
    })

    test("Check the login with blank values", {tag:'@pom1'}, async ({})=>{
    ...
    test("Check the login with blank password", {tag:'@pom1'}, async ({})=>{
        await loginPage.validLogin(username, "")
        await expect(loginPage.requiredFields).toHaveCount(1)
        await expect(loginPage.requiredFields.last()).toHaveText("*Password is required")
    })

    test("Check the login with data in email field without '@gmail.com'", {tag:'@pom1'}, async ({})=>{
    ...
    test("Check the login with data in email field without 'com'", {tag:'@pom1'}, async ({})=>{
        await loginPage.validLogin(invalidUsername1, password)
        await expect(loginPage.requiredFields).toHaveCount(1)
        await expect(loginPage.requiredFields.last()).toHaveText("*Enter Valid Email")
    })

    test("Navigate to Register page using 'Don't have an account? Register here' link", {tag:'@pom1'}, async()=>{
    ...
    test("Navigate to Forgot Password page using 'Forgot password?' link", {tag:'@pom1'}, async()=>{
